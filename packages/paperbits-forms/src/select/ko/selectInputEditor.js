...
var selectInputEditor_html_1 = require("./selectInputEditor.html");
var styles_1 = require("@paperbits/styles");
var selectInputModel_1 = require("../selectInputModel");
var decorators_1 = require("@paperbits/common/ko/decorators");
var SelectEditor = (function () {
    function SelectEditor(styleService) {
    ...
    SelectEditor.prototype.initialize = function () {
        var _a;
    ...
    SelectEditor.prototype.applyChanges = function () {
    ...
    SelectEditor.prototype.addItem = function () {
        var _this = this;
        if (this.itemNameToAdd() !== "" && this.itemValueToAdd() !== "" &&
            !this.options().find(function (item) { return item.value === _this.itemValueToAdd(); })) {
    ...
    SelectEditor.prototype.upItem = function () {
        var selectedFirstValue = this.selectedItems()[0];
        var selectedLastValue = this.selectedItems()[this.selectedItems().length - 1];
        var posFirst = this.options().findIndex(function (item) { return item.value === selectedFirstValue; });
        var posLast = this.options().findIndex(function (item) { return item.value === selectedLastValue; });
        if (posFirst > 0) {
            var moveItem = this.options.splice(posFirst - 1, 1);
            this.options.splice(posLast, 0, moveItem[0]);
            this.onChange(this.model);
        }
    };
    SelectEditor.prototype.downItem = function () {
        var selectedFirstValue = this.selectedItems()[0];
        var selectedLastValue = this.selectedItems()[this.selectedItems().length - 1];
        var posFirst = this.options().findIndex(function (item) { return item.value === selectedFirstValue; });
        var posLast = this.options().findIndex(function (item) { return item.value === selectedLastValue; });
        if (posLast < this.options().length - 1) {
            var moveItem = this.options.splice(posLast + 1, 1);
            this.options.splice(posFirst, 0, moveItem[0]);
            this.onChange(this.model);
        }
    };
    SelectEditor.prototype.deleteItem = function () {
        var _this = this;
        if (this.selectedItems().length > 0) {
            this.options.remove(function (item) { return _this.selectedItems().findIndex(function (selectedValue) { return selectedValue === item.value; }) !== -1; });
            this.onChange(this.model);
            this.selectedItems([]);
        }
    };
    SelectEditor.prototype.setSelectedItemDefault = function () {
        if (this.selectedItems().length === 0) {
            return;
        }
        var selectedValue = this.selectedItems()[0];
        this.model.value = selectedValue;
        this.onChange(this.model);
    };
    __decorate([
        decorators_1.Param(),
        __metadata("design:type", selectInputModel_1.SelectInputModel)
    ], SelectEditor.prototype, "model", void 0);
    __decorate([
        decorators_1.Event(),
        __metadata("design:type", Function)
    ], SelectEditor.prototype, "onChange", void 0);
    __decorate([
        decorators_1.OnMounted(),
        __metadata("design:type", Function),
        __metadata("design:paramtypes", []),
        __metadata("design:returntype", Promise)
    ], SelectEditor.prototype, "initialize", null);
    SelectEditor = __decorate([
        decorators_1.Component({
            selector: "select-input-editor",
            template: selectInputEditor_html_1.default
        }),
